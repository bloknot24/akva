<footer id="footer">
    <div class="section__size">
        <div class="footer__first-level">
            <div class="footer__logo-block">
                <a href="/" class="header__logo-link">
                    <img src="img/header/logo.svg" alt="Лого"
                    class="header__logo-img">
                    <p class="header__logo-text">АкваНика</p>
                </a>
                <div class="footer__confidencial-block">
                    <a href="politika-konfidenczialnosti" class="footer__confidencial-link">
                        Политика конфиденциальности
                    </a>
                </div>
            </div>
            <div class="footer__first-menu">
                <ul class="footer__first-menu-list">
                    <li class="footer__first-menu-item">
                        <a href="/mezhdunarodnye-perevozki" class="footer__menu-item-link">
                            Международные перевозки
                        </a>
                    </li>
                    <li class="footer__first-menu-item">
                        <a href="/perevozki-s-temperaturnym-rezhimom" class="footer__menu-item-link">
                            Перевозки с температурным режимом
                        </a>
                    </li>
                    <li class="footer__first-menu-item">
                        <a href="/peregon-avtotekhniki" class="footer__menu-item-link">
                            Перегон автотехники
                        </a>
                    </li>
                    <!-- <li class="footer__first-menu-item">
                        <a href="#" class="footer__menu-item-link">
                            Ремонт техники
                        </a>
                    </li> -->
                </ul>
            </div>
            <div class="footer__second-menu">
                <ul class="footer__second-menu-list">
                    <li class="footer__second-menu-item">
                        <a href="#" class="footer__menu-item-link">
                            Сертификаты
                        </a>
                    </li>
                    <li class="footer__second-menu-item">
                        <a href="#" class="footer__menu-item-link">
                            Лицензии
                        </a>
                    </li>
                    <li class="footer__second-menu-item">
                        <a href="/#reviews" class="footer__menu-item-link">
                            Отзывы
                        </a>
                    </li>
                    <!-- <li class="footer__second-menu-item">
                        <a href="#" class="footer__menu-item-link">
                            Вакансии
                        </a>
                    </li> -->
                </ul>
            </div>
            <div class="footer__contacts-box">
                <div class="footer__contacts-block">
                    <i class="fas fa-phone-square"></i>
                    <a href="tel:+375296858974" class="footer__contacts-link">
                        +375(29)685-89-74
                    </a>
                </div>
                <div class="footer__contacts-block">
                    <i class="fas fa-phone-square"></i>
                    <a href="tel:+375447777760" class="footer__contacts-link">
                         +375(44)777-77-60
                    </a>
                </div>
                <div class="footer__contacts-block">
                    <i class="fas fa-fax"></i>
                    <a href="tel:80235327728" class="footer__contacts-link">
                         8-02353-277-28
                    </a>
                </div>
                <div class="footer__contacts-block">
                    <i class="fas fa-envelope" style="padding-top: 2px;"></i>
                    <a href="mailto:mail@akvanika.by" class="footer__contacts-link">
                        mail@akvanika.by
                    </a>
                </div>
            </div>
        </div>
        <div class="footer__second-level">
            <div class="vcard">
                <p class="adr">
                    Юр. адрес:
                    <span class="locality">РБ, 247960 г. Житковичи</span>
                    <span class="street-address">ул. Пролетарская д. 81а</span>
                    <span class="fn org">ЧПТУП "Аква Ника"</span>
                </p>
            </div>
            <div class="footer__copyright">
                <p class="footer__copyright-text">
                    <?=date('Y');?> © Все права защищены
                </p>
            </div>
        </div>
    </div>
    <div class="arrow__up" hidden>
        <img src="img/arrow-up.svg" class="arrow__up-img" alt="Стрелка">
    </div>
    <script src="js/arrow-up.js"></script>
</footer>
<script src="js/menu-desc.js"></script>
<script src="js/menu-mobile.js"></script>
<!-- <script src="js/script.js"></script> -->
<script defer src="js/fontawesome.js"></script>
</body>
</html>
